...
min_year, min_engine_displacement, max_price  = search_criteria
filtered_cars = {key: value for key, value in car_data.items() if value[1] >= min_year and value[2] >= min_engine_displacement and value[4] <= max_price}
filtered_cars_sorted_by_price = sorted(filtered_cars.items(), key=lambda item: item[1][4])

print ("ТОП 5 автомобілів, які відповідають критеріям (сортування за збільшенням ціни):")
for car in filtered_cars_sorted_by_price[:5]:
  print(car)